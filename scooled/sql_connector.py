...
class SQLConnector():
    def __init__(self):
        """connect
        """        
        self.conn = None#psycopg2.connect(database=os.environ["db"], user=os.environ["user"],
                        #             host=os.environ["hostname"], password=os.environ["password"], port=os.environ["port"])
    ...
    def connect(self):
        self.conn = psycopg2.connect(database=os.environ["db"], user=os.environ["user"],
                                     host=os.environ["hostname"], password=os.environ["password"], port=os.environ["port"])
        return self.conn

    # @st.cache(hash_funcs={psycopg2.extensions.connection: id},show_spinner=False)
    def query(self, command: str):
        """executes query and closes connection and returns list

        Args:
            command (str): query

        Returns:
            self.conn.cursor values as list: 
        """
        self.connect()
        with self.conn:
    ...
    def get_where_specified(self, table: str, get_cols: list, from_col: list, val_from_col: list, dtype_from_col: list):
        ...
        Args:
            table (str): table name
            get_cols (list): cols to retrieve from
            from_col (list): specified col
            val_from_col (list): specified val
            dtype_from_col (list): data type of from_column data 
        """        
